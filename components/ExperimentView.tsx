...
export const ExperimentView: React.FC<ExperimentViewProps> = ({
  ...
  // Isolated refs to prevent timer clashing
  const stareTimerRef = useRef<any>(null);
  const persistenceTimerRef = useRef<any>(null);

  // --- HOOK 1: Handle the Countdown Interval safely ---
  useEffect(() => {
    if (phase === ExperimentPhase.STARING) {
      setTimeLeft(stareLimit);
      
      stareTimerRef.current = setInterval(() => {
        setTimeLeft((prev) => {
          if (prev <= 1) {
            clearInterval(stareTimerRef.current!);
            return 0; // No longer calling onFinishStare() directly in the render phase
          }
          return prev - 1;
        });
      }, 1000);
    ...
    return () => {
      if (stareTimerRef.current) clearInterval(stareTimerRef.current);
    };
  }, [phase, stareLimit]);

  // --- HOOK 2: Watch the clock and trigger parent safely ---
  useEffect(() => {
    if (phase === ExperimentPhase.STARING && timeLeft === 0) {
      onFinishStare();
    }
  }, [timeLeft, phase, onFinishStare]);

  // --- HOOK 3: Handle the Persistence Observation Timer ---
  useEffect(() => {
